...
class Actor(object):
    ...
    def create_actor_network(self, scope, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            net = self.inp
            net = slim.fully_connected(net, 400, activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    while total_timesteps < args['max_timesteps']:
        ...
        if done:
            ...
            if total_timesteps != 0:
                print("total steps: ", total_timesteps, "episode num: ", episode_num, "episode reward ", episode_reward)
        ...
        if total_timesteps < args['start_timesteps']:
            ...
            action = actor.predict(np.reshape(s, (1, actor.s_dim)))
            action = (action + np.random.normal(0, args['expl_noise'], size=action.shape)).clip(env.action_space.low,
